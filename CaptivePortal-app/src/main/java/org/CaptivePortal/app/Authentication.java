...
public class Authentication {
	...
	public String accessCheck() {
		...
		// if ((src_ip.equals(db_ip) && src_port.equals("3306")) || dst_ip.equals(db_ip)
		// && (dst_port.equals("3306")))
		// return "Pass";
	...
	// String s_url = accessDbUrl + "/insertFlow?src_mac=" + src_mac + "&dst_mac=" +
	// dst_mac +
	// "&src_ip=" + src_ip + "&dst_ip=" + dst_ip + "&src_port=" + src_port +
	// "&dst_port=" + dst_port +
	// "&protocol=" + protocol + "&src_user=" + src_user + "&dst_user=" + dst_user +
	// "&swId=" + in_sw +
	// "&port=" + in_port + "&date=" + date + "&time=" + time + "&src_swId=" +
	// src_access_sw +
	// "&src_access_port=" + src_access_port + "&dst_swId=" + dst_access_sw +
	...
	public void updateBytes(String in_sw, String in_port, String src_mac, String dst_mac, String src_ip, String dst_ip,
		 ...
		 **/
		// try {
		// String s_url = accessDbUrl + "/update_bytes?src_mac=" + src_mac + "&dst_mac="
		// + dst_mac +
		// "&src_ip=" + src_ip + "&dst_ip=" + dst_ip + "&src_port=" + src_port +
		// "&dst_port=" + dst_port +
		// "&protocol=" + protocol + "&swId=" + in_sw + "&port=" + in_port + "&bytes=" +
		// bytes;
		// s_url = s_url.replace(" ","%20");
	...
	private void updateIp() {
		 ...
		 * 
		 * Check whether IP is in IP_MAC table Update MAC of it if IP exists, insert it
		 * otherwise
		 **/
		// try {
	...
	private void insertMac(String mac) {
		 ...
		 **/
		// try {
		// if(!mac.equalsIgnoreCase(portalMac) && !mac.equalsIgnoreCase(gatewayMac)) {
		// String s_url = accessDbUrl + "/insert_mac?mac=" + mac + "&enable=0&switch=" +
		// src_access_sw;
		// s_url = s_url.replace(" ", "%20");
	...
	private boolean macExist(String mac) {
		/**
		 * Check whether MAC exists in Registered_MAC table If it does not exist, insert
		 * it
		 *
		 * @param mac MAC address of the host
		 *
	...
	// **/
	// try {
	// String s_url = accessDbUrl + "/query_acl?group=" + group_id + "&ip=" +
	// dst_ip;
	// s_url = s_url.replace(" ", "%20");
	...
	// int days = 7;
	// String expirationTime = dateTimeFormat.format(new
	// Date(Long.valueOf(this.time) + days*milliSec));
	// String s_url = accessDbUrl + "/update_expirationTime?mac=" + src_mac +
	// "&switch=" + src_access_sw + "&time=" + expirationTime;
	// s_url = s_url.replace(" ", "%20");

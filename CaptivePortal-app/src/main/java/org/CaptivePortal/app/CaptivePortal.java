...
public class CaptivePortal {
	...
	@Property(name = "flowTimeout", intValue = FORWARD_TIMEOUT, label = "Configure Flow Timeout for installed flow rules; "
			+ "default is 20 sec")
	private int flowTimeout = FORWARD_TIMEOUT;

	@Property(name = "flowPriority", intValue = FORWARD_PRIORITY, label = "Configure Flow Priority for installed flow rules; "
			+ "default is 20")
	private int flowPriority = FORWARD_PRIORITY;

	@Property(name = "defaultPriority", intValue = DEFAULT_PRIORITY, label = "Configure Default Flow Priority for installed flow rules; "
			+ "default is 40001")
	...
	public void activate(ComponentContext context) {
		...
		appId = coreService.registerApplication("org.CaptivePortal.app");
		...
		log.info("org.CaptivePortal Started", appId.id());
	}
	...
	public void deactivate() {
		...
		processor = null;
		log.info("org.CaptivePortal Stopped");
	}
	...
	private class ReactivePacketProcessor implements PacketProcessor {
		...
		public void process(PacketContext context) {
			...
			if (ethPkt.getEtherType() == Ethernet.TYPE_IPV4) {
				...
				auth = new Authentication(src_mac.toString(), dst_mac.toString(), src_ip.toString(), dst_ip.toString(),
						src_port, dst_port, protocol, src_access_sw.toString(), src_access_port.toString(),
	...
	private void pktFromPortal(PacketContext context) {
		...
		if (src_ip != null && old_src_ip != null)
	...
	private void installRule(PacketContext context, PortNumber portNumber) {
		...
		// Long queue =
		// bandwidthAllocationService.getQueue(context.inPacket().receivedFrom().deviceId(),
	...
	private class InternalFlowRuleListener implements FlowRuleListener {
		...
		public void event(FlowRuleEvent event) {
			...
			case RULE_REMOVED:
				...
				String src_ip = ((IPCriterion) selector.getCriterion(Criterion.Type.IPV4_SRC)).ip().address()
						.toString();
				String dst_ip = ((IPCriterion) selector.getCriterion(Criterion.Type.IPV4_DST)).ip().address()
						.toString();
				String src_port = "";
				String dst_port = "";
